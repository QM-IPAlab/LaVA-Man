...
from util.pos_embed import get_2d_sincos_pos_embed, get_2d_varsize_sincos_pos_embed
...
class MaskedAutoencoderViTOurs(MaskedAutoencoderViT):
    def __init__(self, img_size=(320,160), patch_size=16, in_chans=3,
                 embed_dim=1024, depth=24, num_heads=16,
                 decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False):
        super().__init__()
        #FIXME: Hardcoded the image_size for now
        ...
        self.initialize_weights_ours()


    def initialize_weights_ours(self):
        # initialization
        # initialize (and freeze) pos_embed by sin-cos embedding
        pos_embed = get_2d_varsize_sincos_pos_embed(self.pos_embed.shape[-1],
            int(self.img_size[0]//16),int(self.img_size[1]//16), cls_token=True)
        self.pos_embed.data.copy_(torch.from_numpy(pos_embed).float().unsqueeze(0))
...
def mae_vit_base_patch16_ours(**kwargs):
    model = MaskedAutoencoderViTOurs(
        patch_size=16, embed_dim=768, depth=12, num_heads=12,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
mae_vit_ours = mae_vit_base_patch16_ours