...
from mae.mpi_models.util.optimization import get_lr_update
from mae.mpi_models.util.transformer import Block, PatchEmbed, RMSNorm, get_2D_position_embeddings, \
                                            MLP, MotionFormerDecoderLayer, ProjectionHead
from mae.mpi_models.util.ms_deform_attn import MultiScaleDeformableAttention
from mae.mpi_models.util.loss_utils import flow_warp, robust_l1, SSIM, get_smooth_loss, xywh2xyxy, get_patches_covered
from mae.mpi_models.util.extraction import TokenAggregation
...
class MPI(nn.Module):
    """
    MPI base refers from https://github.com/OpenDriveLab/MPI/blob/main/mpi/configs/models.py#L38
    """
    def __init__(
        self,
        img_size: int= 224,
    ...
    def unpatchify(self, x):
        p = self.patch2embed.patch_size[0]
        h = self.resolution // p
        w = self.resolution // p
