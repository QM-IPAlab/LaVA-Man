...
class MAERobotLang(nn.Module):
    ...
    def forward_decoder(self, latent1, masked_latent2, ids_restore2, lang_emb):
        """
        latent1: visible
    ...
    def forward(self, img1, img2, pick, place, lang, mask_ratio=0.75):

        # encoder of the first observed image (no mask)
        latent1, mask1, ids_restore2 = self.forward_encoder(img1, mask_ratio=0.0)

        # encoder of the second (goal) image
        # TODO: Make use of the pick and place location in the encoder
...
mae_rl = mae_vit_base_patch16_rl
