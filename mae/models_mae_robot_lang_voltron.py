...
class Voltron(nn.Module):
    """
    """
    def __init__(self, model_name="v-cond", freeze=False):
        super(Voltron, self).__init__()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model, _ = load(model_name, device=self.device, freeze=freeze)

    def forward(self, img1, img2, pick, place, lang, mask_ratio):
        #self.decoder_pos_embed_2 = self.decoder_pos_embed2
        input_ids, attention_mask = lang["input_ids"], lang["attention_mask"]        
        loss, recon, mask, = self.model.forward(img1, input_ids, attention_mask)
