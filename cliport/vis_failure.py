            ...
            for i in range(0, n_demos):
                ...
                for j in range(2):
                    ...
                    for idx in range(task.max_steps):  # 3
                        ...
                        # if testing, calculate total reward
                        if j == 0:
                            total_reward += reward
                            print(f'Lang Goal: {lang_goal}')
                            print(f'Total Reward: {total_reward:.3f} | Done: {done}\n')

                        # if this is the second time, save intermediate results
                        else:
                            print("Save intermediate results.")
                            cache = get_local.cache

                            #image = cache['CLIPLingUNetLat.forward.img'][0]  # torch.Size([1, 3, 320, 320])
                            image = cache['MAESegDPT2LossModel.forward.rgb'][0]  # torch.Size([1, 3, 320, 320])
                            image = vu.tensor_to_cv2_img(image, to_rgb=False)

                            #heatmap = cache['CLIPLingUNetLat.forward.out'][0]  # torch.Size([1, 3, 320, 320])
                            heatmap = cache['MAESegDPT2LossModel.forward.predict'][0]
                            heatmap = heatmap.squeeze()

                            # relevance map
                            #relevance_map = cache['MAESeg2Model.forward.relevance'][0]
                    ...
                    if j == 0:
                        ...
                        if total_reward > 0.9:
                            ...
                            print("Task failed. Recording the failure.")
