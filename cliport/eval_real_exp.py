...
def main(vcfg):
    ...
    while not input('Press q to exit') == "q":
        ...
        heatmap_pick = vu.save_tensor_with_heatmap(img, pick_conf, l=lang_goal, return_img=True, loc=p0_pix)
        show_image(heatmap_pick[:,:,::-1])
        heatmap_place = vu.save_tensor_with_heatmap(img, place_conf, l=lang_goal, return_img=True, loc=p1_pix)
