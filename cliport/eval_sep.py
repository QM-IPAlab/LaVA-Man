...
def main(vcfg):
    ...
    if vcfg['sep'] == True:
        model_path_1 = os.path.join(vcfg['model_path_1'], 'best.ckpt')
        model_path_2 = os.path.join(vcfg['model_path_2'], 'best.ckpt')
    ...
    for train_run in range(vcfg['n_repeats']):
        ...
        # Load checkpoint
        agent.load_sep(model_path_1, model_path_2)
        print(f"Loaded: {model_path_1, model_path_2}")
...
def list_ckpts_to_eval(vcfg, existing_results):
    ...
    # Validation checkpoints that haven't been already evaluated.
        ...
        best_v0_checkpoint = 'best-v0.ckpt'
        if best_checkpoint not in existing_results:  ckpts_to_eval.append(best_checkpoint)
        if last_checkpoint not in existing_results:  ckpts_to_eval.append(last_checkpoint)
        if best_v0_checkpoint not in existing_results:  ckpts_to_eval.append(best_v0_checkpoint)
