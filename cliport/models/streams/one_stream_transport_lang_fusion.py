...
class OneStreamTransportMAEBatchRecon(OneStreamTransportMAEBatch):

    def transport(self, in_tensor, crop, lang):
        logits, loss1 = self.key_stream_one(in_tensor, lang)
        kernel, loss2 = self.query_stream_one(crop, lang)
        return logits, kernel, loss1+loss2
    ...
    def forward(self, inp_img, p, lang_goal, softmax=True):
        ...
        logits, kernel, loss = self.transport(inp_img, cropped_rotated, lang_goal)
        out = self.correlate(logits, kernel, softmax)
        return out, loss